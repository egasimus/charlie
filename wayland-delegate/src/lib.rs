...
fn delegate (
    ...
    quote! {
        impl #generic #crate:Dispatch<#interface, #udata> for #dispatch_from {
            fn request (
                state:     &mut Self,
                client:    &$crate::Client,
                resource:  &$interface,
...
fn delegate_global (
    ...
    quote! {
        impl #generic #crate::GlobalDispatch<#interface, #udata> for #dispatch_from {
            fn bind (
                state:       &mut Self,
                dhandle:     &$crate::DisplayHandle,
                client:      &$crate::Client,
...
#[proc_macro]
pub fn delegate_dispatch (input: TokenStream) -> TokenStream {
}
